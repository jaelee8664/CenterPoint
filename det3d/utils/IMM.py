...
class IMMEstimator():
    ...
    def __init__(self, filters, mu, M):
        ...
        x_shape = filters[0].kf.x.shape
        for f in filters:
            if x_shape != f.kf.x.shape:
                raise ValueError(
                    'All filters must have the same state dimension')

        self.x = zeros(filters[0].kf.x.shape)
        # self.z = np.array([[None]*4]).T
        self.P = zeros(filters[0].kf.P.shape)
    ...
    def _compute_state_estimate(self):
        ...
        for f, mu in zip(self.filters, self.mu):
            self.x = self.x + f.kf.x * mu

        self.P.fill(0)
        for f, mu in zip(self.filters, self.mu):
            y = f.kf.x - self.x
            self.P = self.P + mu * (outer(y, y) + f.kf.P)
